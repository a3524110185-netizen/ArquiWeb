...
import { ArrowLeft, Plus, Trash2, ShoppingCart, Loader2, AlertCircle } from 'lucide-react';
...
export default function EditarPedidoPage() {
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      const [c, m, pedido] = await Promise.all([
      ...
      if (pedido.estado.toLowerCase() !== 'pendiente') {
        toast.error('No editable', 'Solo se pueden editar pedidos en estado pendiente');
        router.replace(`/comercial/pedidos/${id}`);
        return;
      }

      setClienteId(String(pedido.cliente_id));
      setFechaEntrega(pedido.fecha_entrega ? pedido.fecha_entrega.slice(0, 10) : '');
      setObservaciones(pedido.observaciones || '');
      setDetalles((pedido.detalles || []).map((d, i) => ({
        tempId: `${d.material_id}-${i}`,
        material_id: d.material_id,
        nombre: d.material?.nombre || `Material #${d.material_id}`,
        cantidad: d.cantidad,
  ...
  useEffect(() => {
    if (currentUser && !canManage) { router.replace('/comercial/pedidos'); return; }
  ...
  const handleGuardar = async () => {
    ...
    setSubmitting(true);
    try {
      await pedidosService.actualizar(id, {
      ...
      router.push(`/comercial/pedidos/${id}`);
    } catch (err: any) {
      toast.error('Error', extractErrorMessage(err, 'No se pudo actualizar el pedido'));
    } finally {
      setSubmitting(false);
    }
  };
